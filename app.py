...
def main():
    ...
    if uploaded_files:
        ...
        for file in uploaded_files:
            try:
                if file.name.endswith('.xlsx'):
                    ...
                    # Detect Swiggy Metadata and skip it
                    peek = pd.read_csv(file, nrows=1)
                    if "Selected Filters" in peek.columns[0]:
        ...
        if all_dfs:
            ...
            st.download_button("📥 Download Final Strategy", data=buffer.getvalue(), file_name="swiggy_instamart_strategy.xlsx")
